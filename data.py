...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        return (
            torch.tensor(
                pad_series(
                    self.df_list[index][self.features].values.astype(
                        np.float32),
                    self.seq_len,
                ),
            ),
            torch.tensor(
                int(self.df_list[index][self.class_col].iloc[0])).float(),
        )
...
class DataModule(pl.LightningDataModule):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        if setting_config is None:
            raise ValueError(
                "Setting configuration is missing in the provided config.")
        if trainer_config is None:
            raise ValueError(
                "Trainer configuration is missing in the provided config.")
